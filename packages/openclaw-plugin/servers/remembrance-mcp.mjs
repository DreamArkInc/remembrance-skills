...
function remembranceConfigPath(env = process.env, homeDirectory = homedir3()) {
  const configRoot = env.REMEMBRANCE_PLUGIN_HOST?.trim().toLowerCase() === "openclaw" ? join3(homeDirectory, ".config") : join3(homeDirectory, ".config");
...
function sharedConfigStatus(env = process.env, fileSystem = nodeFileSystem2) {
  const path = remembranceConfigPath(env);
  const location = env.REMEMBRANCE_PLUGIN_HOST?.trim().toLowerCase() !== "openclaw" && false ? "custom_shared_config" : "default_shared_config";
  if (!fileSystem.exists(path)) {
    return {
      location,
      present: false,
...
var SERVER_VERSION = true ? "0.1.77" : "0.0.0-dev";
